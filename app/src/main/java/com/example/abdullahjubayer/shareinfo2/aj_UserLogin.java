...
public class aj_UserLogin extends AppCompatActivity {
    ...
    private void userLogin() {
        if (!userLoginValid()){
            Toast.makeText(aj_UserLogin.this,"Please Fill All Data Correctly",Toast.LENGTH_LONG).show();
        }
        else {
    ...
    private void userValid(final String email, final String pass) {
            ...
            user.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    ...
                    if (!doc.exists()){
                        Toast.makeText(aj_UserLogin.this,"Wrong Email",Toast.LENGTH_LONG).show();
                    }else {
                ...
                public void onFailure(@NonNull Exception e) {
                    progressBar.setVisibility(View.INVISIBLE);
                    Toast.makeText(aj_UserLogin.this,"Company Not Found",Toast.LENGTH_LONG).show();
                }
            });
    ...
    private boolean userLoginValid() {
        ...
        if (ad_email.isEmpty() || !android.util.Patterns.EMAIL_ADDRESS.matcher(ad_email).matches()) {
            email.setError("enter a valid aj_admin email address");
            val = false;
        } else {
            email.setError(null);
        }
